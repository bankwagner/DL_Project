...
if __name__ == "__main__":
    ...
    cm_results = create_cm(opt_weighted_ensemble_prediction, optimal_weights, y_test)
    
    with open("ensemble_optimal_weights.txt", "w") as f:
        f.write(f"Best single model's IOU: {best_single_model_iou}\n")
        f.write(f"Ensemble performance is {improvement_percentage:.3f}% better than the best model alone (improvement in IOU: {improvement_absolute:.3f} with weights {optimal_weights})\n")
        f.write(f"Confusion matrix: \n {cm_results[0]}\n")
        f.write(f"\nAccuracy: {cm_results[1]}\n")
        f.write(f"Precision: {cm_results[2]}\n")
        f.write(f"Recall: {cm_results[3]}\n")
        f.write(f"F1 Score: {cm_results[4]}\n")
