...
def parse_arguments():
    ...
    parser.add_argument('--model', choices=['unet', 'linknet', 'fpn'], help='Specify the model to use: "unet", "linknet", or "fpn".', required=True)
    parser.add_argument('--traintype', choices=['pre-trained', 'self-trained'], default='pre-trained',
...
if __name__ == "__main__":
    ...
    model = load_model(f'/content/{args.model}_{args.traintype}.h5', compile=False)
    ...
    create_prediction_plot(x_test, y_test, model, args.model, args.traintype)
    
    create_test_metrics(x_test, y_test, model, args.model, args.traintype)
